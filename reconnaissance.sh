...
copy_snapshot_files() {
  ...
  copy_tree /etc/caddy "$SNAP/etc/caddy"
  ...
  copy_tree /etc/selinux "$SNAP/etc/selinux"
...
main() {
...

  {
    ...
    if [[ -n "$AUTHORIZED_USERS_FILE" ]]; then echo "authorized_users_file=$AUTHORIZED_USERS_FILE"; else echo "authorized_users_file=not_provided"; fi
    if [[ "$IS_ROOT" != "yes" ]]; then
...
ss -tulpn | egrep ":22|:53|:80|:443|:8080|:8000|:3000|:5000|:8443|:3306|:5432" || true
echo

echo "[Likely web stack]"
ps auxww | egrep -i "nginx|apache|httpd|caddy|php-fpm|node|pm2|gunicorn|uwsgi|tomcat|flask|django|rails|puma" | grep -Ev "egrep|grep -E -i" || true
echo

echo "[Likely web/app roots]"
find /var/www /srv /opt -maxdepth 5 \
  \( -path "/opt/VBoxGuestAdditions-*" -o -path "/root/ecitadel_www_repo" -o -path "/root/ecitadel_www_repo/*" -o -path "/home/*/ecitadel_www_repo" -o -path "/home/*/ecitadel_www_repo/*" \) -prune -o \
  -type f \( -name "index.*" -o -name "app.py" -o -name "wsgi.py" -o -name "manage.py" -o -name "package.json" -o -name "composer.json" -o -name "wp-config.php" -o -name "settings.py" -o -name "*.service" \) -print 2>/dev/null | head -200 || true
echo

echo "[Immediate sensitive-file red flags under app roots]"
find /var/www /srv /opt -maxdepth 8 \
  \( -path "/opt/VBoxGuestAdditions-*" -o -path "/root/ecitadel_www_repo" -o -path "/root/ecitadel_www_repo/*" -o -path "/home/*/ecitadel_www_repo" -o -path "/home/*/ecitadel_www_repo/*" \) -prune -o \
  -type f \( -name ".env" -o -name ".env.*" -o -name "*.sqlite" -o -name "*.sqlite3" -o -name "*.db" -o -name "*.jsonl" -o -name "*.bak" -o -name "*.old" -o -name "*.save" \) -ls 2>/dev/null | head -200 || true
'
...
id
echo "EUID=$EUID"
if [ "$EUID" -ne 0 ]; then echo "INCOMPLETE: not running as root"; else echo "Running as root"; fi
echo
...
ss -tanp 2>/dev/null | head -180 || true
echo

echo "[DNS]"
cat /etc/resolv.conf 2>/dev/null || true
command -v resolvectl >/dev/null 2>&1 && resolvectl status 2>/dev/null | head -160 || true
'
...
ps auxww | egrep -i "apache|nginx|httpd|caddy|php|php-fpm|node|npm|pm2|gunicorn|uwsgi|tomcat|flask|django|python|perl|ruby|java|ssh|cron|sssd|realmd|firewalld|named|unbound|dnsmasq" | grep -Ev "egrep|grep -E -i" || true
echo

echo "[Top processes]"
ps -eo user,pid,ppid,stat,%cpu,%mem,etime,cmd --sort=-%cpu | head -90 || true
echo

echo "[Relevant installed packages]"
if command -v dpkg-query >/dev/null 2>&1; then
  dpkg-query -W 2>/dev/null | egrep -i "nginx|apache|httpd|openssh|php|python3-flask|gunicorn|uwsgi|nodejs|npm|mysql|mariadb|postgres|redis|tomcat|sssd|realmd|samba|krb5|ufw|iptables|nftables|firewalld|selinux|auditd|bind|named|dnsmasq|unbound" || true
elif command -v rpm >/dev/null 2>&1; then
  rpm -qa | egrep -i "nginx|apache|httpd|openssh|php|python.*flask|gunicorn|uwsgi|nodejs|npm|mysql|mariadb|postgres|redis|tomcat|sssd|realmd|samba|krb5|ufw|iptables|nftables|firewalld|selinux|audit|bind|named|dnsmasq|unbound" || true
fi
'
...
getent passwd 2>/dev/null | awk -F: "\$7 ~ /(bash|sh|zsh|ksh)$/ {print \$1 ":" \$7}" | head -500 || true
echo
...
if [ -n "$AUTHORIZED_USERS_FILE" ] && [ -f "$AUTHORIZED_USERS_FILE" ]; then
  tmp_current=$(mktemp); tmp_auth=$(mktemp)
...
for f in /etc/ssh/sshd_config.d/*.conf; do [ -f "$f" ] && echo "--- $f ---" && sed -n "1,220p" "$f"; done
echo
...
echo "[sssd.conf redacted preview]"
if [ -f /etc/sssd/sssd.conf ]; then
  sed -E "s/(password|secret|keytab|token|bind_pw)[[:space:]]*=.*/\1 = REDACTED/Ig" /etc/sssd/sssd.conf 2>/dev/null | head -220 || true
fi
echo
...
echo "[Domain-aware groups of interest]"
getent group sudo 2>/dev/null || true
getent group wheel 2>/dev/null || true
getent group "domain admins" 2>/dev/null || true
getent group "rrintel admins" 2>/dev/null || true
echo

echo "[Domain-related sudo/SSH references]"
grep -RInE "sudo|wheel|admin|domain|sss|ldap|krb5|AllowUsers|AllowGroups" /etc/sudoers /etc/sudoers.d /etc/sssd /etc/ssh 2>/dev/null | head -400 || true
'
...
for u in $(cut -d: -f1 /etc/passwd 2>/dev/null); do
  ...
  if [ -n "$out" ]; then echo "--- crontab for $u ---"; echo "$out"; fi
done
echo

echo "[Systemd timers/services modified recently]"
if [ -d /run/systemd/system ]; then systemctl list-timers --all --no-pager || true; else echo "systemd unavailable."; fi
echo
find /etc/systemd/system /lib/systemd/system /usr/lib/systemd/system -type f -mtime -30 -ls 2>/dev/null | head -400 || true
'
...
find / -xdev \
  ...
  -perm -4000 -type f -ls 2>/dev/null | head -400 || true
echo

echo "[SGID files]"
find / -xdev \
  \( -path "/root/ecitadel_www_repo" -o -path "/root/ecitadel_www_repo/*" -o -path "/home/*/ecitadel_www_repo" -o -path "/home/*/ecitadel_www_repo/*" \) -prune -o \
  -perm -2000 -type f -ls 2>/dev/null | head -400 || true
echo

echo "[Linux file capabilities]"
command -v getcap >/dev/null 2>&1 && getcap -r / 2>/dev/null | egrep -v "/ecitadel_www_repo/" | head -400 || echo "getcap unavailable"
echo

echo "[Executable files in risky writable/app locations]"
find /tmp /var/tmp /dev/shm /var/www /srv /opt \
  \( -path "/opt/VBoxGuestAdditions-*" -o -path "/tmp/VSLiveshareLogs/*" -o -path "/tmp/python-languageserver-cancellation/*" -o -path "/tmp/pyright-*/*" -o -path "/root/ecitadel_www_repo" -o -path "/root/ecitadel_www_repo/*" -o -path "/home/*/ecitadel_www_repo" -o -path "/home/*/ecitadel_www_repo/*" \) -prune -o \
  -type f -perm /111 -ls 2>/dev/null | head -400 || true
echo
...
if command -v dpkg >/dev/null 2>&1; then dpkg -V 2>/dev/null | egrep -v "Permission denied|/boot/System.map|/boot/vmlinuz" | head -400 || true; else echo "dpkg not installed."; fi
echo

echo "[Debian/Ubuntu: debsums -C]"
if command -v debsums >/dev/null 2>&1; then debsums -C 2>/dev/null | head -400 || true; else echo "debsums not installed."; fi
echo

echo "[Fedora/RHEL: rpm -Va]"
if command -v rpm >/dev/null 2>&1; then rpm -Va 2>/dev/null | head -500 || true; else echo "rpm not installed."; fi
echo

echo "[Fedora/RHEL: dnf installed relevant packages]"
if command -v dnf >/dev/null 2>&1; then dnf list installed 2>/dev/null | egrep -i "nginx|httpd|apache|openssh|php|python|nodejs|npm|mysql|mariadb|postgres|redis|tomcat|sssd|realmd|samba|krb5|firewalld|nftables|iptables|selinux|audit|bind|named|dnsmasq|unbound" | head -500 || true; else echo "dnf unavailable"; fi
'
...
echo
echo "[nftables]"
if command -v nft >/dev/null 2>&1; then nft -a list ruleset 2>/dev/null || true; else echo "nft unavailable"; fi

echo
echo "[iptables active rules]"
if command -v iptables >/dev/null 2>&1; then iptables -S 2>/dev/null || true; echo; iptables-save -c 2>/dev/null || true; else echo "iptables unavailable"; fi

echo
echo "[ip6tables active rules]"
if command -v ip6tables >/dev/null 2>&1; then ip6tables -S 2>/dev/null || true; echo; ip6tables-save -c 2>/dev/null || true; else echo "ip6tables unavailable"; fi

echo
echo "[ufw status]"
if command -v ufw >/dev/null 2>&1; then ufw status numbered verbose 2>&1 || true; echo; ufw show raw 2>&1 || true; else echo "ufw unavailable"; fi
...
if command -v ausearch >/dev/null 2>&1; then
  ausearch -m AVC,USER_AVC -ts recent 2>/dev/null | tail -200 || true
else
  grep -RInaE "avc:|USER_AVC|denied" /var/log/audit/audit.log /var/log/messages 2>/dev/null | tail -200 || echo "ausearch unavailable and no readable audit/messages AVC lines"
fi
'

  section "10_web_stack" '
echo "[Web-related processes]"
ps auxww | egrep -i "apache|nginx|httpd|caddy|php|php-fpm|node|npm|pm2|gunicorn|uwsgi|tomcat|flask|django|rails|puma" | grep -Ev "egrep|grep -E -i" || true
echo
...
if [ -d /run/systemd/system ]; then
  systemctl status nginx httpd apache2 caddy php-fpm 'php*-fpm' tomcat gunicorn uwsgi --no-pager 2>/dev/null || true
else
...
find /var/www /srv /opt -maxdepth 6 \
  \( -path "/opt/VBoxGuestAdditions-*" -o -path "/root/ecitadel_www_repo" -o -path "/root/ecitadel_www_repo/*" -o -path "/home/*/ecitadel_www_repo" -o -path "/home/*/ecitadel_www_repo/*" \) -prune -o \
  -type f \( -name "index.*" -o -name "package.json" -o -name "app.py" -o -name "wsgi.py" -o -name "manage.py" -o -name "requirements.txt" -o -name "composer.json" -o -name "wp-config.php" -o -name "settings.py" -o -name "*.service" \) -print 2>/dev/null | head -400 || true
'
...
if command -v nginx >/dev/null 2>&1 || pgrep -x nginx >/dev/null 2>&1 || [ -d /etc/nginx ]; then
  ...
  if command -v nginx >/dev/null 2>&1; then
    ...
    nginx -T 2>&1 | egrep -n "server_name|listen|root |alias |index |try_files|proxy_pass|fastcgi_pass|uwsgi_pass|location |auth_basic|allow |deny |autoindex|client_max_body_size|access_log|error_log|include " | head -700 || true
  else
    echo "nginx config evidence exists, but nginx binary was not found."
  fi
else
  echo "nginx not detected; skipping nginx checks."
fi

echo
if command -v apache2ctl >/dev/null 2>&1 || command -v apachectl >/dev/null 2>&1 || pgrep -x apache2 >/dev/null 2>&1 || pgrep -x httpd >/dev/null 2>&1 || [ -d /etc/httpd ] || [ -d /etc/apache2 ]; then
  echo "[Apache/httpd active/configured: config test]"
  ...
  command -v httpd >/dev/null 2>&1 && httpd -t 2>&1 || true
  echo
  echo "[Apache/httpd important config references]"
  grep -RInE "^(Listen|ServerName|DocumentRoot|DirectoryIndex|Alias|ProxyPass|ProxyPassReverse|CustomLog|ErrorLog|Include|IncludeOptional|LoadModule|Require|AllowOverride|Options|SetHandler|AddHandler)" /etc/apache2 /etc/httpd 2>/dev/null | head -700 || true
  echo
  echo "[Apache/httpd enabled sites/modules summary]"
  ls -la /etc/apache2/sites-enabled /etc/apache2/mods-enabled /etc/httpd/conf.d /etc/httpd/conf.modules.d 2>/dev/null || true
else
...
if command -v caddy >/dev/null 2>&1 || pgrep -x caddy >/dev/null 2>&1 || [ -d /etc/caddy ]; then
  ...
  command -v caddy >/dev/null 2>&1 && caddy validate --config /etc/caddy/Caddyfile 2>&1 || echo "caddy binary not found; skipping validation."
...
find /var/www /srv /opt -maxdepth 8 \
  \( -path "/opt/VBoxGuestAdditions-*" -o -path "/root/ecitadel_www_repo" -o -path "/root/ecitadel_www_repo/*" -o -path "/home/*/ecitadel_www_repo" -o -path "/home/*/ecitadel_www_repo/*" \) -prune -o \
  -type f \( -name "*.jsonl" -o -name "*.log" -o -name "*.db" -o -name "*.sqlite" -o -name "*.sqlite3" -o -name ".env" -o -name ".env.*" -o -name "*config*.php" -o -name "settings.py" -o -name "wp-config.php" -o -name "*.bak" -o -name "*.old" -o -name "*.save" \) -ls 2>/dev/null | head -400 || true
echo

echo "[World-writable web/app files]"
find /var/www /srv /opt \
  \( -path "/opt/VBoxGuestAdditions-*" -o -path "/root/ecitadel_www_repo" -o -path "/root/ecitadel_www_repo/*" -o -path "/home/*/ecitadel_www_repo" -o -path "/home/*/ecitadel_www_repo/*" \) -prune -o \
  -type f -perm -0002 -ls 2>/dev/null | head -400 || true
echo

echo "[World-writable web/app directories]"
find /var/www /srv /opt \
  \( -path "/opt/VBoxGuestAdditions-*" -o -path "/root/ecitadel_www_repo" -o -path "/root/ecitadel_www_repo/*" -o -path "/home/*/ecitadel_www_repo" -o -path "/home/*/ecitadel_www_repo/*" \) -prune -o \
  -type d -perm -0002 -ls 2>/dev/null | head -400 || true
echo

echo "[Recently modified web/app files]"
find /var/www /srv /opt \
  \( -path "/opt/VBoxGuestAdditions-*" -o -path "/root/ecitadel_www_repo" -o -path "/root/ecitadel_www_repo/*" -o -path "/home/*/ecitadel_www_repo" -o -path "/home/*/ecitadel_www_repo/*" \) -prune -o \
  -type f -mtime -7 -ls 2>/dev/null | head -700 || true
'

  section "13_web_logs_routes" '
echo "[Suspicious web log entries]"
grep -RInaE " 40[0-9] | 50[0-9] |\.env|\.git|/etc/passwd|union select|select.+from|base64|cmd=|\.php\?|\.cgi|\.jsp|\.asp|%2e%2e|\.\./|curl|wget|xmlrpc|wp-admin|PROPFIND" /var/log/nginx /var/log/apache2 /var/log/httpd 2>/dev/null | tail -700 || true
echo

echo "[Top requested paths]"
awk "{print \$7}" /var/log/nginx/access.log /var/log/nginx/access.log.* /var/log/apache2/access.log /var/log/apache2/access.log.* /var/log/httpd/access_log /var/log/httpd/access_log.* 2>/dev/null | sort | uniq -c | sort -nr | head -80 || true
echo

echo "[Top client IPs]"
awk "{print \$1}" /var/log/nginx/access.log /var/log/nginx/access.log.* /var/log/apache2/access.log /var/log/apache2/access.log.* /var/log/httpd/access_log /var/log/httpd/access_log.* 2>/dev/null | sort | uniq -c | sort -nr | head -50 || true
echo
...
if [ -d /run/systemd/system ]; then journalctl -u ssh -u sshd --since "24 hours ago" --no-pager 2>/dev/null | tail -500 || true; else echo "systemd unavailable."; fi
echo

echo "[Auth log high-signal lines]"
grep -RInaE "Failed password|Accepted password|Accepted publickey|Invalid user|authentication failure|sudo:|session opened|session closed" /var/log/auth.log /var/log/secure 2>/dev/null | tail -500 || true
echo

echo "[lastb if available]"
lastb -a 2>/dev/null | head -80 || true
'
...
find '$SNAP' -maxdepth 3 -print 2>/dev/null | sed 's#^$SNAP#.#' | sort | head -450
echo
